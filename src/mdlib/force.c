	...
	for(i=0;i<md->homenr;i++)
	  {
	    ...
	    dR+=(fx-R1)*(fx-R1)+(fy-R2)*(fy-R2)+(fz-R3)*(fz-R3);
	    sumR+=(R1)*(R1)+(R2)*(R2)+(R3)*(R3);
	    ...
	    if(to_write)
	      ...
	      ret = fscanf(g2,"%lf %lf %lf\n",&F1,&F2,&F3);

	    dF+=(fx-F1)*(fx-F1)+(fy-F2)*(fy-F2)+(fz-F3)*(fz-F3);
	    sumF+=(F1)*(F1)+(F2)*(F2)+(F3)*(F3);
	    */
	    if(only_Fourier)
	      {
		fx = f_long[i][XX];
