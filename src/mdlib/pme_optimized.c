// This routine is edited to exert Parallel Spectral Ewald
// Davoud Saffar Shamshirgar
// The followings are edited to carry out SE and PME in this routine
// SSE routines are included. AVX routines are included
// src/kernel/mdrun
...
#define GMX_CACHE_SEP 32

#define PME_ORDER_MAX 40
...
static void pme_realloc_splinedata(splinedata_t *spline, pme_atomcomm_t *atc)
{
  ...
  //    srenew(spline->zs, (atc->pme_order-1)*atc->nalloc);
  int p = atc->pme_order;
...
void PME_SE_grid_split_d(real *grid, real *q,
			 splinedata_t *spline,
...
// -----------------------------------------------------------------------------
void PME_SE_grid_split(real* grid, real* q,
		       splinedata_t *spline,
		       const SE_FGG_params* params)
{
  // unpack parameters
...
void PME_SE_grid_split_SSE_P16_d(real *grid, real *q,
				 splinedata_t *spline,
				 const SE_FGG_params *params)
{
  // unpack parameters
...
void PME_SE_grid_split_SSE_P8_d(real *grid, real *q,
				splinedata_t *spline,
				const SE_FGG_params *params)
{
  // unpack parameters
	  ...
	  for(i = 0; i<8; i++)
	    {
	      for(j = 0; j<8; j++)
		{
		  rC = _mm_set1_pd( qn*zx[8*n+i]*zy[8*n+j] );
		  idx_zz=8*n;
...
void PME_SE_grid_split_SSE_u8_d(real *grid, real *q,
				splinedata_t *spline,
				const SE_FGG_params *params)
{
  // unpack parameters
...
// -----------------------------------------------------------------------------
void PME_SE_grid_split_SSE_d(real *grid, real *q,
			     splinedata_t *spline,
			     const SE_FGG_params *params)
{
  // unpack parameters
...
// -----------------------------------------------------------------------------
void PME_SE_grid_split_SSE(real* grid, real* q,
			   splinedata_t *spline,
			   const SE_FGG_params* params)
{
  // unpack parameters
...
// -----------------------------------------------------------------------------
void PME_SE_grid_split_SSE_P8(real *grid, real *q,
			      splinedata_t *spline,
			      const SE_FGG_params *params)
{
  // unpack parameters
...
// -------------------------------------------------------------------------
void PME_SE_grid_split_AVX_P16_d(real *grid, real* q,
				 splinedata_t *spline,
				 const SE_FGG_params* params)
{
  // unpack parameters
...
// -----------------------------------------------------------------------------
void PME_SE_grid_split_AVX_P8_d(real* grid, real* q,
				splinedata_t *spline,
				const SE_FGG_params* params)
{
  // unpack parameters
...
// -----------------------------------------------------------------------------
void PME_SE_grid_split_AVX_u8_d(real* grid, real* q,
				splinedata_t *spline,
				const SE_FGG_params* params)
{
  // unpack parameters
...
// -----------------------------------------------------------------------------
void PME_SE_grid_split_AVX_d(real* grid, real* q,
			     splinedata_t *spline, 
			     const SE_FGG_params* params)
{
  // unpack parameters
...
// -----------------------------------------------------------------------------
void PME_SE_grid_split_AVX(real* grid, real* q,
			   splinedata_t *spline, 
			   const SE_FGG_params* params)
{
  // unpack parameters
...
// -----------------------------------------------------------------------------
void
PME_SE_grid_split_SSE_dispatch_d(real *grid, real *q,
				 splinedata_t *spline,
				 const SE_FGG_params *params)
{
  const int p = params->P;
  ...
  if( is_odd(p) || is_odd(incri) || is_odd(incrj) )
    {
      ...
      PME_SE_grid_split_d(grid, q, spline, params);
      return;
    }
  ...
  if(p==16)
    {
      ...
      PME_SE_grid_split_SSE_P16_d(grid, q, spline, params);
    }
  else if(p==8)
    {
      // specific for p divisible by 8
      __DISPATCHER_MSG("[FGG GRID SSE] P=8\n");
      PME_SE_grid_split_SSE_P8_d(grid, q, spline, params); 
    }  
  else if(p%8==0)
    {
      // specific for p divisible by 8
      __DISPATCHER_MSG("[FGG GRID SSE] P unroll 8\n");
      PME_SE_grid_split_SSE_u8_d(grid, q, spline, params); 
    }
  else
    {
      // vanilla SSE code (any even p)
      __DISPATCHER_MSG("[FGG GRID SSE] Vanilla\n");
      PME_SE_grid_split_SSE_d(grid, q, spline, params);
    }
}
...
// ---------------------------------------------
void
PME_SE_grid_split_AVX_dispatch_d(real* grid, real* q, 
				 splinedata_t *spline,
				 const SE_FGG_params* params)
{
  const int p = params->P;
  ...
  if( isnot_div_by_4(p) || isnot_div_by_4(incri) || isnot_div_by_4(incrj) )
    {
      ...
      //	PME_SE_grid_split_SSE_d(grid, q, spline, params);
      PME_SE_grid_split_SSE_dispatch_d(grid, q, spline, params);
      return;
    }
  ...
  if(p==16)
    {
      ...
      PME_SE_grid_split_AVX_P16_d(grid, q, spline, params); 
    }
  else if(p==8)
    {
      // specific for p=8
      __DISPATCHER_MSG("[FGG GRID AVX] P=8\n");
      PME_SE_grid_split_AVX_P8_d(grid, q, spline, params); 
    }
  else if(p%8==0)
    {
      // specific for p divisible by 8
      __DISPATCHER_MSG("[FGG GRID AVX] P unroll 8\n");
      PME_SE_grid_split_AVX_u8_d(grid, q, spline, params); 
    }
  else if(p%4==0)
    {
      // specific for p divisible by 4
      __DISPATCHER_MSG("[FGG GRID AVX] P unroll 4\n");
      PME_SE_grid_split_AVX_d(grid, q, spline, params); 
    }
  else
    {
      // vanilla SSE code (any even p)
...
void 
PME_SE_grid_split_SSE_dispatch(real* grid, real* q, 
			       splinedata_t *spline,
			       const SE_FGG_params* params)
{
  const int p = params->P;
  ...
  if( isnot_div_by_4(p) || isnot_div_by_4(incri) || isnot_div_by_4(incrj)  || (p%4)!=0)
    {
      __DISPATCHER_MSG("[FGG GRID SSE] SSE Abort (PARAMS)\n");
      PME_SE_grid_split(grid, q, spline, params);
      return;
    }
  ...
  if(p==8)
    {
      // specific for p=8
      __DISPATCHER_MSG("[FGG GRID SSE] P=8\n");
      PME_SE_grid_split_SSE_P8(grid, q, spline, params);
    }
  else
    {
      // vanilla SSE code for p divisible by 4
      __DISPATCHER_MSG("[FGG GRID SSE] Vanilla\n");
      PME_SE_grid_split_SSE(grid, q, spline, params);
    }
}
...
// -----------------------------------------------------------------------------
void 
PME_SE_grid_split_AVX_dispatch(real* grid, real* q, 
			       splinedata_t *spline,
			       const SE_FGG_params* params)
{
  const int p = params->P;
  ...
  if( isnot_div_by_8(p) || isnot_div_by_8(incri) || isnot_div_by_8(incrj) || (p%8)!=0)
    {
      __DISPATCHER_MSG("[FGG GRID AVX] AVX Abort (PARAMS)\n");
      PME_SE_grid_split_SSE_dispatch(grid, q, spline, params);
      return;
    }
  ...
  // otherwise the preconditions for AVX codes are satisfied. 
  else if(p==8)
    {
      // specific for p=8
      __DISPATCHER_MSG("[FGG GRID AVX] P=8\n");
      PME_SE_grid_split_AVX(grid, q, spline, params); 
    }
}
#endif
				     ...
				     gmx_bool          se_set)
{
  ...
  if(se_set)
    {
#if defined(GMX_X86_SSE2) && !defined(GMX_X86_AVX_256)
#if defined GMX_DOUBLE
      PME_SE_grid_split_SSE_dispatch_d(grid,atc->q,spline,se_params);
#else
      PME_SE_grid_split_SSE_dispatch(grid,atc->q,spline,se_params);
#endif
#elif defined(GMX_X86_AVX_256)
#if defined GMX_DOUBLE
      PME_SE_grid_split_AVX_dispatch_d(grid,atc->q,spline,se_params);
#else
      PME_SE_grid_split_AVX_dispatch(grid,atc->q,spline,se_params);
#endif
#endif
    }
  else
  ...
  if (ptr == NULL)
    {
      ...
      //davoud
#if defined GMX_X86_AVX_256
      snew_aligned(grid->grid, gridsize, 32);
#else
...

{
  ...
  // davoud as soon as epsilon_r=1 no worries
  ...
  if(se_set)
      ...
      if (bEnerVir)
	{
	  ...
	  // FIXME: what is this value coming from in the energy? (dss)
...
void PME_SE_int_split_d(rvec *force, real *grid, real *q,
			splinedata_t *spline,
...
// -----------------------------------------------------------------------------
void PME_SE_int_split(rvec* force,  real* grid, real* q,
		      splinedata_t *spline,
		      const SE_FGG_params* params, real scale, 
		      gmx_bool bClearF)
{
...
void PME_SE_int_split_SSE_d(rvec *force, real *grid, real *q,
			    splinedata_t *spline,
			    const SE_FGG_params *params, real scale,
			    gmx_bool bClearF)
{
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_SSE_P8_d(rvec *force, real *grid, real *q,
			       splinedata_t *spline,
			       const SE_FGG_params *params, real scale,
			       gmx_bool bClearF)
{
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_SSE_P16_d(rvec *force, real *grid, real *q,
				splinedata_t *spline,
				const SE_FGG_params *params, real scale,
				gmx_bool bClearF)
{
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_SSE_u8_d(rvec *force, real *grid, real *q,
			       splinedata_t *spline,
			       const SE_FGG_params *params, real scale,
			       gmx_bool bClearF)
{
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_SSE_P8(rvec *force, real *grid, real *q,
			     splinedata_t *spline,
			     const SE_FGG_params *params, real scale,
			     gmx_bool bClearF)
{
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_SSE(rvec *force, real *grid, real *q,
			  splinedata_t *spline,
			  const SE_FGG_params *params, real scale,
			  gmx_bool bClearF)
{
...
#ifdef GMX_X86_AVX_256
void PME_SE_int_split_AVX_d(rvec *force, real *grid, real *q,
			    splinedata_t *spline,
			    const SE_FGG_params *params, real scale,
			    gmx_bool bClearF)
{
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_AVX_P8_d(rvec* force, real* grid, real* q,  
			       splinedata_t *spline,
			       const SE_FGG_params* params, real scale,
			       gmx_bool bClearF)
{
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_AVX_P16_d(rvec* force, real* grid, real* q,  
				splinedata_t *spline,
				const SE_FGG_params* params, real scale,
				gmx_bool bClearF)
{
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_AVX_u8_d(rvec* force, real* grid, real* q,  
			       splinedata_t *spline,
			       const SE_FGG_params* params, real scale,
			       gmx_bool bClearF)
{
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_AVX(rvec* force,  real* grid, real* q,
			  splinedata_t *spline,
			  const SE_FGG_params* params, real scale,
			  gmx_bool bClearF)
{
...
void PME_SE_int_split_SSE_dispatch_d(rvec *force, real *grid, real *q,
				     splinedata_t *spline,
				     const SE_FGG_params *params, real scale,
				     gmx_bool bClearF)
{
  ...
  if( is_odd(p) || is_odd(incri) || is_odd(incrj) )  
    {
      ...
      PME_SE_int_split_d(force, grid, q, spline, params, scale, bClearF);
      return;
    }
  // otherwise the preconditions for SSE codes are satisfied. 
  if(p==8)
    {
      ...
      PME_SE_int_split_SSE_P8_d(force, grid, q, spline, params, scale, bClearF);
    }
  else if(p==16)
    {
      // specific for p=16
      __DISPATCHER_MSG("[FGG INT SSE] P=16\n");
      PME_SE_int_split_SSE_P16_d(force, grid, q, spline, params, scale, bClearF); 
    }
  else if(p%8==0)
    {
      // for p divisible by 8
      __DISPATCHER_MSG("[FGG INT SSE] P unroll 8\n");
      PME_SE_int_split_SSE_u8_d(force, grid, q, spline, params, scale, bClearF); 
    }
  else
    {
      // vanilla SSE code (any even p)
      __DISPATCHER_MSG("[FGG INT SSE] Vanilla\n");
      PME_SE_int_split_SSE_d(force, grid, q, spline, params, scale, bClearF);
    }
}
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_AVX_dispatch_d(rvec* force, real* grid, real* q, 
				     splinedata_t *spline,
				     const SE_FGG_params* params, real scale,
				     gmx_bool bClearF)
{
  ...
  if( isnot_div_by_4(p) || isnot_div_by_4(incri) || isnot_div_by_4(incrj) )
    {
      ...
      //	PME_SE_int_split_d(force, grid, q, spline, params, scale, bClearF);
      PME_SE_int_split_SSE_dispatch_d(force, grid, q, spline, params, scale, bClearF);
      return;
    }
  ...
  if(p==8)
    {
      ...
      PME_SE_int_split_AVX_P8_d(force, grid, q, spline, params, scale, bClearF);
    }
  else if(p==16)
    {
      // specific for p=16
      __DISPATCHER_MSG("[FGG INT AVX] P=16\n");
      PME_SE_int_split_AVX_P16_d(force, grid, q, spline, params, scale, bClearF);
    }
  else if(p%8==0)
    {
      // for p divisible by 8
      __DISPATCHER_MSG("[FGG INT AVX] P unroll 8\n");
      PME_SE_int_split_AVX_u8_d(force, grid, q, spline, params, scale, bClearF); 
    }
  else if(p%4==0)
    {
      // vanilla AVX code (p divisible by 4)
      __DISPATCHER_MSG("[FGG INT AVX] P unroll 4\n");
      PME_SE_int_split_AVX_d(force, grid, q, spline, params, scale, bClearF);
    }
  else
    {
      // vanilla SSE code (any even p)
...
// -----------------------------------------------------------------------------
void PME_SE_int_split_SSE_dispatch(rvec *force, real *grid, real *q,
				   splinedata_t *spline,
				   const SE_FGG_params *params, real scale,
				   gmx_bool bClearF)
{
  ...
  if( isnot_div_by_4(p) || isnot_div_by_4(incri) || isnot_div_by_4(incrj)  || (p%4)!=0 )
    {
      __DISPATCHER_MSG("[FGG INT SSE] SSE Abort (PARAMS)\n");
      PME_SE_int_split(force, grid, q, spline, params, scale, bClearF);
      return;
    }
  ...
  if(p==8)
    {
      // specific for p=8
      __DISPATCHER_MSG("[FGG INT SSE] P=8\n");
      PME_SE_int_split_SSE_P8(force, grid, q, spline, params, scale, bClearF);
    } 
  else
    {
      // vanilla SSE code for p divisible by 4
      __DISPATCHER_MSG("[FGG INT SSE] Vanilla\n");
      PME_SE_int_split_SSE(force, grid, q, spline, params, scale, bClearF);
    }
}
...
// ----------------------------------------------------------------------------
void PME_SE_int_split_AVX_dispatch(rvec* force, real* grid, real* q,
				   splinedata_t *spline,
				   const SE_FGG_params* params, real scale,
				   gmx_bool bClearF)
{
  ...
  if( isnot_div_by_8(p) || isnot_div_by_8(incri) || isnot_div_by_8(incrj) || (p%8)!=0)
    {
      __DISPATCHER_MSG("[FGG INT AVX] AVX Abort (PARAMS)\n");
      PME_SE_int_split_SSE_dispatch(force, grid, q, spline, params, scale, bClearF);
      return;
    }
  // otherwise the preconditions for AVX codes are satisfied. 
  if(p==8)
    {
      // specific for p=8
      __DISPATCHER_MSG("[FGG INT AVX] P=8\n");
      PME_SE_int_split_AVX(force, grid, q, spline, params, scale, bClearF);
    }
}
#endif //AVX
...
#if  defined(GMX_X86_SSE2) && !defined(GMX_X86_AVX_256)
#if defined GMX_DOUBLE
      PME_SE_int_split_SSE_dispatch_d(atc->f, grid, atc->q, spline, se_params, 
				      scale, bClearF);
#else
      PME_SE_int_split_SSE_dispatch(atc->f, grid, atc->q, spline, se_params, 
				    scale, bClearF);
#endif
#elif defined(GMX_X86_AVX_256)
#if defined GMX_DOUBLE
      PME_SE_int_split_AVX_dispatch_d(atc->f, grid, atc->q, spline, se_params, 
				      scale, bClearF);
#else
      PME_SE_int_split_AVX_dispatch(atc->f, grid, atc->q, spline, se_params, 
				    scale, bClearF);
#endif
#endif
    }
		   ...
		   pme_atomcomm_t * atc)
{
  ...
  if(se_set)
    {
      // davoud fill zs
      PME_SE_FGG_base_gaussian(zs, se_params);
      for (i = 0; i < nr; i++)
       {
          ii = ind[i];
  ...
  for (thread = 0; thread < nthread; thread++)
    {
      ...
      // davoud calculate zs, zx, zy, zz, zfx, zfy, zfz
      // calls base_gaussian and expand_all
      if (bCalcSplines)
	{
	  make_bsplines(spline->theta, spline->dtheta, pme->pme_order,
			atc->fractx, spline->n, spline->ind, atc->q, pme->bFEP,
			spline->zs, spline->idx, atc->x, se_params,se_set
			,atc);
	}
	  ...
	  ct1a = omp_cyc_start();
#endif
	  spread_q_bsplines_thread(grid, atc, spline, pme->spline_work, se_params, se_set);
...
void gmx_pme_calc_energy(gmx_pme_t pme, int n, rvec *x, real *q, real *V)
{
  ...
  spread_on_grid(pme, atc, NULL, TRUE, FALSE, pme->fftgridA, NULL, false);
  ...
  for (q = 0; q < (pme->bFEP ? 2 : 1); q++)
    {
      ...
      if(se_set)
	{
	  parse_params(&se_opt, ewaldcoeff);
	  ...
	  /* Spread the charges on a grid */
	  // davoud
	  spread_on_grid(pme, &pme->atc[0], pmegrid, q == 0, TRUE, fftgrid, &se_params, se_set);
	  // davoud SUSPICIOUS
	  //    grid = tempgrid->grid;
	  ...
	  if (pme->nthread == 1)
		...
		for (j=0;j<8;j++)
		for (k=0;k<10;k++)
		ss+=fftgrid[i*8*10+j*10+k]*fftgrid[i*8*10+j*10+k];
		printf("FFTGRID %f\n",ss);
	      */
            }
	  if(se_set)
	...
	if (flags & GMX_PME_SOLVE)
	  {
	    ...
	    loop_count =
	      solve_pme_yzx(pme, cfftgrid, ewaldcoeff,
			    ...
			    bCalcEnerVir,
			    pme->nthread, thread, &se_params, se_set);
	...
	if (bCalcF)
	  {
	    ...
	    /*
	      ...
	      for (j=0;j<8;j++)
	      for (k=0;k<10;k++)
	      ss+=fftgrid[i*8*10+j*10+k]*fftgrid[i*8*10+j*10+k]/(8*8*8)/(8*8*8);
	      printf("FFTGRID %f\n",ss);
	    */
	    if (thread == 0)
	      {
	    ...
	    // sesum(grid,2,0,128,3,"after_fft");
	  }
      }
...
#pragma omp parallel for num_threads(pme->nthread) schedule(static)
  ...
  /*
    ...
    if(nid!=0)
    MPI_Send(&ss, 1, MPI_DOUBLE, 0, 101,atc->mpi_comm);
    else
    {
    for (i=1;i<pme->nnodes;i++){
    MPI_Recv(&ss,1, MPI_DOUBLE, i, 101, atc->mpi_comm, &stat);
    stot += ss;
    }
    }
    GMX_BARRIER(atc->mpi_comm);
    }
  */
#endif
